...
#' @examples 
#' \dontrun{
#' data <- get_baseline(creat, .blood = blood_data)
#' }
#' @export

get_baseline <- function(.data, labvalue, patientid, result_day, .blood = NULL) {
  labvalue <- rlang::enquo(labvalue)
  
  class(.data) <- dplyr::case_when(stringr::str_detect(rlang::as_label(labvalue), "bili|creatinine")~ append(class(.data), "bili"),
...
#' @rdname get_baseline
#' @export
get_baseline.bili <- function(.data, labvalue, patientid, result_day, .blood = NULL) {
  labvalue <- rlang::enquo(labvalue)
...
#' @rdname get_baseline
#' @export
get_baseline.plate <- function(.data, labvalue, patientid, result_day, .blood = NULL) {
  labvalue <- rlang::enquo(labvalue)
  patientid <- rlang::enquo(patientid)
  result_day <- rlang::enquo(result_day)
