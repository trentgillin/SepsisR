...
#' @param new_name Character variable that indicates what you want to the time difference variable to be called.
#' @param unitx Character variable that indicates what units to return time difference, default is days.
...
calc_time_between <- function(dataset, begining_time, end_time, new_name = "time_diff", unitx = "days") {
  ...
  if (is.Date(dataset[[beginings_time]]) |
    ...
    dataset <- dplyr::mutate(dataset, time_diff_1 = lubridate::as.difftime(!!end_time - !!begining_time))
